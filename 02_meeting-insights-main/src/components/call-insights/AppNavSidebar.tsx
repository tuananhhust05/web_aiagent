...
const navItems: NavItem[] = [
  {
    ...
    href: "https://mockflow-artist.lovable.app",
  },
  {
    icon: Video,
    label: "Meeting Insight",
    href: "https://concept-artisan-space.lovable.app",
  },
  { icon: Target, label: "Strategy", action: "strategy-modal" },
  {
    icon: ClipboardCheck,
    label: "Action Ready",
    href: "https://smooth-build-plan.lovable.app",
  },
  {
    icon: HelpCircle,
    label: "QnA Engine",
    href: "https://visual-dreamer-box.lovable.app",
  },
  {
    icon: BarChart3,
    label: "Performance",
    href: "https://page-wonderland-79.lovable.app",
  },
];
...
const settingsSubItems: SettingsSubItem[] = [
  {
    icon: BookOpen,
    label: "Knowledge",
    href: "https://knowledge-forskale.lovable.app",
  },
  {
    icon: RecordIcon,
    label: "Record",
    href: "https://record-forskale.lovable.app",
  },
    ...
    <>
      ...
      >
        ...
        <div className={cn("px-3 pt-1.5 pb-2", "flex justify-center")}>
          <button
            className={cn(
              "forskale-home-capsule group relative flex items-center gap-3 rounded-2xl px-5 py-3 text-sm font-semibold text-white transition-all duration-200",
              collapsed ? "justify-center px-3" : "justify-center",
            )}
            onClick={() => window.location.href = "https://home-forskale.lovable.app"}
          >
            ...
            {!collapsed && <span className="tracking-wide drop-shadow-[0_0_8px_hsl(197,86%,64%,0.3)]">Home</span>}
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 py-1 space-y-0.5 overflow-y-auto overflow-x-hidden">
          {navItems.map((item) => {
            ...
            return item.href ? (
              <a key={item.label} href={item.href} target="_self" className={classes}>
                {content}
              </a>
            ) : (
