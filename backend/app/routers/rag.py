...
async def update_agent_config(
    ...
    try:
        print("key ...", settings.ELEVENLABS_API_KEY)
        print("agent_id ...", settings.ELEVENLABS_AGENT_ID)
        print("updating with:", {"language": language, "prompt": prompt, "first_message": first_message})
        ...
        if language is None and prompt is None and first_message is None:
            raise HTTPException(
                status_code=400,
                detail="At least one field (language, prompt, first_message) must be provided"
            )
        ...
        if language is not None:
            ...
            # Auto-update TTS model based on language
            if language == "en":
                # English uses eleven_turbo_v2_2
                update_payload["conversation_config"]["tts"] = {
