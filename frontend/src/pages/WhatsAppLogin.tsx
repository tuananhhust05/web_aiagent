...
import { AlertCircle, CheckCircle2, Image as ImageIcon, Loader2, Info } from 'lucide-react'
...
const WhatsAppLogin = () => {
  ...
  const createProfile = async () => {
    ...
    try {
      await whatsappAPI.createLoginProfile()
      setProfileStatus('success')
    } catch (error: any) {
      console.error('Failed to create WhatsApp profile', error)
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs font-medium text-blue-600 uppercase tracking-[0.2em]">WhatsApp</p>
            <h1 className="text-xl font-bold text-gray-900">WhatsApp Login Connector</h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full bg-emerald-500 px-6 py-2 text-sm font-semibold text-white shadow-md hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-60 transition"
            >
              {!userId ? (
                <>
                ...
                <p className="text-gray-500 font-medium">
                  {userId
                    ? imageReady
                      ...
                      : 'Press Login to load your WhatsApp QR code.'
