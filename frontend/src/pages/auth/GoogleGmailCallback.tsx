...
/**
 * Google Gmail OAuth: Google redirects here (frontend URL).
 * This page redirects the browser to the BACKEND with the same ?state=&code=&scope=
 * so the backend can exchange code for tokens and redirect to /atlas/todo?gmail_connected=success.
 * Uses BACKEND_OAUTH_BASE_URL (VITE_BACKEND_OAUTH_URL in prod) to avoid redirecting to same page (infinite loop).
 */
const GoogleGmailCallback: React.FC = () => {
  const [redirectError, setRedirectError] = useState<string | null>(null)

  useEffect(() => {
    const search = window.location.search
    const base = (BACKEND_OAUTH_BASE_URL || '').replace(/\/$/, '')
    const backendCallbackUrl = `${base}/auth/google/gmail/callback${search}`
  ...
  if (redirectError === 'same_origin') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-6">
        ...
        <h1 className="text-lg font-semibold text-gray-900 mb-2">Gmail connection misconfiguration</h1>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
      ...
      <p className="text-sm text-gray-600">Completing Gmail connection…</p>
