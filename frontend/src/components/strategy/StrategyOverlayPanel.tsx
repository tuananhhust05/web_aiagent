...
import { strategyAPI, type StrategyBriefing } from '@/lib/api';
...
interface Props {
  ...
  isFirstLoad: boolean;
  onClose: () => void;
  cachedBriefing: StrategyBriefing | null;
  onBriefingGenerated: (briefing: StrategyBriefing) => void;
  onRerun: () => void;
}
...
export default function StrategyOverlayPanel({ deal, isFirstLoad, onClose, cachedBriefing, onBriefingGenerated, onRerun }: Props) {
  ...
  const [briefing, setBriefing] = useState<StrategyBriefing | null>(cachedBriefing);
  const [visible, setVisible] = useState(false);
  const pendingBriefing = useRef<StrategyBriefing | null>(null);

  const cogState = getCognitiveState(
    briefing ? briefing.interest_score : deal.interestLevel
  );
  const displayInterest = briefing ? briefing.interest_score : deal.interestLevel;
  const displayStageName = briefing ? briefing.cognitive_stage : cogState.name;

  useEffect(() => {
    requestAnimationFrame(() => setVisible(true));
  }, []);

  // Orbital step sequencer + backend API call (run in parallel)
  useEffect(() => {
    if (!isLoading) return;

    pendingBriefing.current = null;
    ...
    // Resolve once both animation and API are complete
    const tryFinalize = () => {
      if (!animationDone || !apiFetchDone) return;
      if (pendingBriefing.current) {
        setBriefing(pendingBriefing.current);
        onBriefingGenerated(pendingBriefing.current);
      }
      setIsLoading(false);
    };

    // Start backend API call immediately (parallel with animation)
    strategyAPI.generateBriefing(deal.id)
      .then(res => {
        pendingBriefing.current = res.data;
      })
      .catch(err => {
        console.error('[StrategyOverlayPanel] Backend briefing failed:', err);
      })
      .finally(() => {
        apiFetchDone = true;
        tryFinalize();
  ...
  }, [isLoading, deal.id, onBriefingGenerated]);

  const handleRerun = useCallback(async () => {
    onRerun();
    try {
      await strategyAPI.clearCache(deal.id);
    } catch {
      // non-fatal — proceed with reload anyway
    }
    setBriefing(null);
    setIsLoading(true);
  }, [onRerun, deal.id]);
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-card shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className={cn("text-xs font-medium", getStageTextColor(displayInterest))}>
              {displayStageName} · {displayInterest}%
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        {isLoading ? (
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="rounded-xl border border-[hsl(var(--forskale-teal)/0.2)] bg-[hsl(var(--forskale-teal)/0.04)] p-5">
              ...
              {briefing.adaptation_strategy && (
                <p className="mt-3 text-sm text-muted-foreground border-t border-border/50 pt-3">
                  <span className="font-semibold text-foreground">Next stage: </span>
                  {briefing.adaptation_strategy}
                </p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center gap-2 mb-2">
                <div className={cn("w-3 h-3 rounded-full", getStageBgColor(displayInterest))} />
                <span className={cn("text-sm font-bold", getStageTextColor(displayInterest))}>{displayStageName}</span>
                <span className="text-xs text-muted-foreground">· {displayInterest}%</span>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed">{briefing.cognitive_state_explanation}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {briefing.risk_warnings.length > 0 && (
                <div className="rounded-xl border border-border bg-card p-4">
                  ...
                  <ul className="space-y-1.5">
                    {briefing.risk_warnings.map((r, i) => (
                      <li key={i} className="text-xs text-muted-foreground flex gap-1.5">
            ...
            {/* AI Task Cards (from backend guardrail engine) */}
            {briefing.task_cards.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-1">
                  <Sparkles className="h-4 w-4 text-[hsl(var(--forskale-teal))]" />
                  <h4 className="text-sm font-bold text-foreground">AI Strategy Tasks</h4>
                </div>
                {briefing.task_cards.map(card => {
                  const priorityColor = card.priority === 'Critical'
                    ? 'text-destructive border-destructive/30 bg-destructive/5'
                    : card.priority === 'High'
                      ? 'text-orange-500 border-orange-500/30 bg-orange-500/5'
                      : 'text-[hsl(var(--forskale-teal))] border-[hsl(var(--forskale-teal)/0.3)] bg-[hsl(var(--forskale-teal)/0.05)]';
                  return (
                    <div key={card.id} className={cn("rounded-xl border p-4", priorityColor)}>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <p className="text-sm font-semibold text-foreground leading-snug">{card.title}</p>
                        <span className={cn("shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full border", priorityColor)}>
                          {card.priority}
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground mb-2">{card.reasoning}</p>
                      <div className="flex flex-wrap gap-2 mb-3 text-[11px]">
                        <span className="font-medium text-foreground">Impact: <span className="text-[hsl(var(--forskale-teal))]">{card.interest_impact}</span></span>
                        <span className="text-muted-foreground">·</span>
                        <span className="font-medium text-foreground">{card.timing_constraint}</span>
                      </div>
                      {card.how_steps.length > 0 && (
                        <ol className="space-y-1">
                          {card.how_steps.map((step, si) => (
                            <li key={si} className="text-xs text-muted-foreground flex gap-1.5">
                              <span className="shrink-0 font-bold text-foreground">{si + 1}.</span> {step}
                            </li>
                          ))}
                        </ol>
                      )}
                      <p className="mt-2 text-[11px] text-muted-foreground/70 italic">✓ {card.success_criteria}</p>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Fallback: show StrategyActionItems from deal data when no task_cards */}
            {briefing.task_cards.length === 0 && <StrategyActionItems deal={deal} />}

            {deal.interestSignals.length > 0 && (
              <CollapsibleSection title={t('section.interestSignals')} icon={Sparkles}>
                <InterestSignals signals={deal.interestSignals} interestLevel={displayInterest} />
              </CollapsibleSection>
            )}

            {/* Rule of One Blocker in DecisionFriction section */}
            {(deal.decisionFriction.length > 0 || briefing.rule_of_one_blocker) && (
              <CollapsibleSection title={t('section.decisionFriction')} icon={Shield}>
                {briefing.rule_of_one_blocker && (
                  <div className="mb-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <AlertTriangle className="h-3.5 w-3.5 text-destructive" />
                      <span className="text-xs font-bold text-destructive">Primary Blocker (Rule of One)</span>
                    </div>
                    <p className="text-sm font-semibold text-foreground mb-1">{briefing.rule_of_one_blocker.title}</p>
                    <p className="text-xs text-muted-foreground mb-2">{briefing.rule_of_one_blocker.why_it_matters}</p>
                    <ul className="space-y-1">
                      {briefing.rule_of_one_blocker.how_to_remove.map((s, i) => (
                        <li key={i} className="text-xs text-muted-foreground flex gap-1.5">
                          <span className="text-[hsl(var(--forskale-teal))]">→</span> {s}
                        </li>
                      ))}
                    </ul>
                  </div>
