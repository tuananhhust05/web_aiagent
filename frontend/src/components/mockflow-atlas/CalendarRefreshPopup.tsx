...
export function CalendarRefreshPopup({ onDismissPermanent }: CalendarRefreshPopupProps) {
  return (
    <div className="absolute left-1/2 top-16 z-50 -translate-x-1/2 animate-in fade-in slide-in-from-top-2 pointer-events-auto">
      <div className="flex items-start gap-3 rounded-lg border border-forskale-teal/20 bg-cal-slate px-4 py-3 shadow-lg backdrop-blur-sm max-w-md">
        ...
        <button className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded text-cal-text-secondary hover:text-cal-text-primary" onClick={onDismissPermanent}>
