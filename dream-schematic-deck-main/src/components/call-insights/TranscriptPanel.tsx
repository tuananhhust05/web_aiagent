...
const TranscriptPanel = ({ entries, collapsed, onToggle, meetingId = "1", isNewTranscript = true }: TranscriptPanelProps) => {
  ...
  return (
    <div
      className={cn("h-screen border-l border-border bg-card flex flex-col overflow-hidden", collapsed ? "w-[56px] min-w-[56px]" : "w-[340px] min-w-[340px]")}
    ...
    >
          ...
          <div className="flex justify-center p-3">
            <div className="relative">
              <Button
                variant="ghost"
                size="icon"
                className={cn("h-10 w-10 rounded-lg bg-accent hover:bg-accent/80 text-muted-foreground hover:text-foreground transition-all duration-150 hover:scale-105 focus-visible:ring-2 focus-visible:ring-[hsl(var(--forskale-teal))]", showGlow && "transcript-glow-active")}
              ...
              {showNewBadge && (
                <span className="absolute -top-1 -right-1 h-4 min-w-[16px] px-0.5 bg-[hsl(var(--forskale-teal))] text-white text-[8px] font-bold rounded-full flex items-center justify-center leading-none">NEW</span>
              )}
            </div>
          </div>
          {/* Change 6: Only vertical label, no audio icons */}
          <div className="flex-1 flex flex-col items-center gap-5 pt-4">
            <span className="text-[10px] text-muted-foreground font-medium tracking-wider uppercase" style={{ writingMode: "vertical-rl" }}>Transcript</span>
          </div>
        </>
      ) : (
        <>
          {/* Change 6: Removed Play and duration from expanded header */}
          <div className={cn("p-4 border-b border-border", hasExpanded && "transcript-header-reveal")}>
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground focus-visible:ring-2 focus-visible:ring-[hsl(var(--forskale-teal))]" onClick={handleCollapse} aria-expanded="true" aria-label="Collapse transcript panel">
          ...
          <div className={cn("px-4 pb-3 pt-3", hasExpanded && "transcript-search-reveal")}>
            <div className="relative">
              ...
              <Input ref={searchRef} placeholder="Search transcript" value={search} onChange={(e) => { setSearch(e.target.value); setActiveTag(null); }} className="pl-8 h-8 text-xs bg-secondary border-border focus:border-[hsl(var(--forskale-teal))] focus:ring-[hsl(var(--forskale-teal)/0.1)]" />
            </div>
          </div>

          <div className={cn("flex gap-2 flex-wrap px-4 py-3 border-b border-border", hasExpanded && "transcript-search-reveal")}>
            {keywords.map((tag) => (
              <button key={tag} onClick={() => handleTagClick(tag)} className={cn("px-2.5 py-1 rounded-full text-xs font-medium transition-colors border", activeTag === tag ? "bg-[hsl(var(--forskale-teal)/0.1)] text-[hsl(var(--forskale-teal))] border-[hsl(var(--forskale-teal)/0.3)]" : "bg-secondary text-muted-foreground border-border hover:bg-accent hover:text-foreground")}>{tag}</button>
            ))}
          </div>
            ...
            {filtered.map((entry, i) => (
              <div key={i} className="flex gap-3">
                <div className="mt-1.5 shrink-0">
                  <div className={cn("w-2 h-2 rounded-full", entry.color === "blue" ? "bg-[hsl(var(--forskale-green))]" : "bg-[hsl(var(--forskale-cyan))]")} />
                </div>
      ...
      <div className="sr-only" aria-live="polite">{collapsed ? "Transcript panel collapsed" : "Transcript panel expanded"}</div>
    </div>
  );
};
