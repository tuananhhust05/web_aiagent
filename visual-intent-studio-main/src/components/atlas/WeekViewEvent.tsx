...
import { getMeetingSequence, VELOCITY_DOT_CLASSES } from "@/lib/meetingSequence";
      ...
      {isShort ? (
        ...
        <>
          ...
          <div className="absolute bottom-1.5 right-1.5 flex items-center gap-1">
            {(() => {
              const participants = MEETING_PARTICIPANTS[meeting.id];
              if (!participants || participants.length <= 1) return null;
              const shown = participants.slice(0, 3);
              return (
                <div className="flex -space-x-1.5">
                  {shown.map((p) => (
                    <div key={p.id} className="h-5 w-5 rounded-full bg-primary/15 border-2 border-card flex items-center justify-center">
                      <span className="text-[7px] font-bold text-primary">{p.name.split(" ").map(n => n[0]).join("")}</span>
                    </div>
                  ))}
            ...
            {meeting.type !== "internal" && (
              <div
                className={cn(
                  ...
                  VELOCITY_DOT_CLASSES[seq.velocity]
                )}
              />
            )}
          </div>
